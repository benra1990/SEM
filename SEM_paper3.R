...
#db_1<-read.xlsx("H:/SIG/Procesos SIG/BD_inequity/base de datos/database.xlsx")
str(db_short)
data.table(colnames(db_short))
#attach(db_short)

#hypothesis: land size inequality and human agency mediate the relationship between ES yield and income.We will test two models, number 1 were income is the outcome variable and one were ES supply inequality is the outcome variable.
...
ginis<-dplyr::select(db_short, starts_with("gini"))##Gini coefficients
income<-db_short[c(55,57)]##mean income & gini income
area<-db_short[,52]##mean of all the properties within a municipality
data.table(colnames(db_short))
...
for (i in 1:39) {
  hist(db[,c(1:39)][,i],main=names(db [,c(1:39)])[i],xlab=names(db[,c(1:39)])[i])
}
dev.off()
...
#rescale data-> I am not using dbn here to see how the analysis looks like without normalized data, so for rescaling the normalized database replace here "db" by "dbn"
#db<-lapply(db[,c(1:40)], scales::rescale)#rescaling data 0 to 1
#db<-as.data.frame(db)

dbn<-lapply(db[,c(1:41)], scales::rescale)#rescaling data 0 to 1
dbn<-as.data.frame(dbn)
...
#check correlations between variables (only human agency variables, because we include correlations of this latent variable in the model)
cor_datos<-cor(db[,2:8], use="pairwise.complete.obs", method="spearman")
cor_matrix<-rcorr(as.matrix(db[,2:8]))

#to check variable order
...
#!!!investigate what orthogonal means!!
...
parameterEstimates(model1a_total_reg_fit)#gives the currently use parameters of the model fit
...
write.xlsx(model1_yield, "H:/SIG/Procesos SIG/Spatial distribution/Tables/model1_yield.xlsx", row.names=TRUE,overwrite=TRUE)#
